...
export async function GET(req: Request): Promise<Response> {
  ...
  for (const merchant of merchants) {
    // Per-merchant guard (like sweep-outcomes): one throwing send/update must not
    // abort the batch and starve every later merchant of its reminder.
    try {
      ...
      console.log(
        JSON.stringify({
          event: "cron.trial-reminders.merchant-failed",
          merchantId: merchant.id,
